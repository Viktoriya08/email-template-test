...
function getParams(data: Record<string, unknown> | unknown[], parentKey = ''): Params {
	...
	Object.entries(data).forEach(([key, value]) => {
		if (isArrayOrObject(value)) { result.push(...getParams(value, getKey(key, parentKey))); }

		else if (isBoolean(value)) { result.push([getKey(key, parentKey), encodeURIComponent(value ? 'Y' : 'N')]); }

		else if (isDate(value)) { result.push([getKey(key, parentKey), encodeURIComponent(value.toISOString())]); }
		...
		else if (!isNull(value) && !isUndefined(value)) { result.push([getKey(key, parentKey), encodeURIComponent(value as string)]); }
	});
